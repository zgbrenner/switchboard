...
import {
  enumValue, exactKeys, normalizeModelInventory, object, optionalBoolean, parseBoosts, parseBudget,
  parseContext, parseFiles, stringValue,
} from './validators.mjs';
...
export function normalizeRouteArguments(value) {
  ...
  const currentModelId = args.currentModelId === undefined
    ? undefined
...
export function normalizeComparisonArguments(value) {
  ...
  const base = {
    prompt: args.prompt, context: args.context, files: args.files, availableModels: args.availableModels,
    currentModelId: args.currentModelId, budget: args.budget, planMode: args.planMode,
  };
...
export function normalizeEvaluationArguments(value) {
  ...
  const cases = args.cases.map((raw, index) => {
    ...
    if (!Array.isArray(requiredCapabilities) || requiredCapabilities.length > CAPABILITIES.length || requiredCapabilities.some((capability) => !CAPABILITIES.includes(capability)) || new Set(requiredCapabilities).size !== requiredCapabilities.length) {
      throw new Error(`cases[${index}].requiredCapabilities is invalid.`);
    }
    const normalized = normalizeRouteArguments({
      prompt: item.prompt, context: item.context, files: item.files, policy: item.policy,
      profile: item.profile, planMode: 'single',
    });
    return {
