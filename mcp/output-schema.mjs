...
const learningSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['applied', 'bias'],
  properties: {
    applied: { type: 'boolean' },
    bias: { type: 'number', minimum: -0.35, maximum: 0.35 },
    reason: { type: 'string' },
    fromTier: { type: 'string', enum: TIERS },
    toTier: { type: 'string', enum: TIERS },
  },
};
...
export const ROUTE_OUTPUT_SCHEMA_V05 = {
  ...
  properties: {
    ...
    compatibility: {
      type: 'object',
      additionalProperties: false,
      required: ['additiveFrom', 'responseContract'],
    ...
    confidenceEvidence: {
      ...
      properties: Object.fromEntries(
        ['overall', 'scoreMargin', 'deterministicEvidence', 'capabilityCertainty', 'agreement'].map((key) => [
          key,
    ...
    executionPlan: {
      ...
      properties: {
        ...
        stages: { type: 'array', minItems: 1, maxItems: 4, items: stageSchema },
      },
    },
    budgetAssessment: {
      type: 'object',
      ...
      properties: {
        ...
        violations: {
          type: 'array',
          uniqueItems: true,
          items: { type: 'string', enum: ['cost', 'latency', 'quality', 'modelRequirements'] },
        },
