...
export const ROUTE_OUTPUT_SCHEMA_V05 = {
  ...
  required: [
    ...
    'apiVersion', 'compatibility', 'effectiveProfile', 'effectivePolicy', 'confidenceEvidence', 'executionPlan',
    'budgetAssessment', 'modelResolution',
  ],
  properties: {
    tier: { type: 'string', enum: TIERS },
    effort: { type: 'string', enum: EFFORTS },
    ...
    scores: {
      ...
      properties: Object.fromEntries(TIERS.map((tier) => [tier, { type: 'number', minimum: 0, maximum: 1 }])),
    },
    taskCategories: { type: 'array', items: { type: 'string' } },
    apiVersion: { type: 'string', enum: ['2026-07-24'] },
    compatibility: {
      type: 'object', additionalProperties: false, required: ['additiveFrom', 'responseContract'],
      properties: { additiveFrom: { type: 'string', const: '0.4.0' }, responseContract: { type: 'string', const: '2026-07-24' } },
    },
    effectiveProfile: { type: 'string', enum: PROFILE_NAMES },
    effectivePolicy: { type: 'string', enum: ['best', 'balanced', 'fast', 'conserve'] },
    confidenceEvidence: {
      type: 'object', additionalProperties: false,
      required: ['overall', 'scoreMargin', 'deterministicEvidence', 'capabilityCertainty', 'agreement'],
      properties: Object.fromEntries(['overall', 'scoreMargin', 'deterministicEvidence', 'capabilityCertainty', 'agreement'].map((key) => [key, { type: 'number', minimum: 0, maximum: 1 }])),
    },
    executionPlan: {
      type: 'object', additionalProperties: false, required: ['mode', 'stages'],
      properties: { mode: { type: 'string', enum: ['single', 'multi'] }, stages: { type: 'array', minItems: 1, maxItems: 4, items: stageSchema } },
    },
    budgetAssessment: {
      type: 'object', additionalProperties: false,
      required: ['fits', 'violations', 'estimatedRelativeCost', 'estimatedRelativeLatency', 'estimatedQuality', 'constraints'],
      properties: {
        fits: { type: 'boolean' },
        violations: { type: 'array', uniqueItems: true, items: { type: 'string', enum: ['cost', 'latency', 'quality'] } },
        estimatedRelativeCost: { type: 'number', minimum: 0, maximum: 1 },
        estimatedRelativeLatency: { type: 'number', minimum: 0, maximum: 1 },
        estimatedQuality: { type: 'number', minimum: 0, maximum: 1 },
        constraints: {
          type: 'object', additionalProperties: false,
          required: ['maxRelativeCost', 'maxRelativeLatency', 'minQuality', 'maxStages'],
          properties: {
