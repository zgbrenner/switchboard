...
function acceptsRequiredTypes(value) {
  const types = new Set(String(value ?? '').toLowerCase().split(',').map((item) => item.split(';', 1)[0].trim()));
...
function plain(request, response, status, text, extraHeaders = {}) {
  response.writeHead(status, { 'content-type': 'text/plain; charset=utf-8', 'cache-control': 'no-store', ...corsHeaders(request), ...extraHeaders });
  response.end(text);
}
...
export function startHttpServer(options = {}) {
  ...
  if (!Number.isInteger(maxSessions) || maxSessions < 1 || maxSessions > 10_000) throw new Error('maxSessions must be an integer between 1 and 10000.');
  ...
  const server = createServer(async (request, response) => {
    ...
    if (!hostAllowed(request.headers.host, host, allowedHosts)) { plain(request, response, 403, 'Forbidden host'); return; }
    if (!originAllowed(header(request.headers.origin), allowedOrigins)) { plain(request, response, 403, 'Forbidden origin'); return; }
    ...
    if (request.method === 'GET') { response.writeHead(405, { allow: 'POST, DELETE, OPTIONS', ...corsHeaders(request) }).end(); return; }

    if (request.method === 'DELETE') {
      const id = sessionHeader(request);
      const protocol = protocolHeader(request);
      if (!id || !protocol) { plain(request, response, 400, 'MCP-Session-Id and MCP-Protocol-Version are required.'); return; }
      const state = sessions.get(id);
      if (!state) { response.writeHead(404, corsHeaders(request)).end(); return; }
      if (protocol !== state.protocolVersion) { plain(request, response, 400, 'MCP protocol version does not match the session.'); return; }
      sessions.delete(id);
      response.writeHead(204, { 'cache-control': 'no-store', ...corsHeaders(request) }).end();
      return;
    }

    if (request.method !== 'POST') { response.writeHead(405, { allow: 'POST, DELETE, OPTIONS', ...corsHeaders(request) }).end(); return; }
    if (!acceptsRequiredTypes(request.headers.accept)) { plain(request, response, 406, 'Accept must include application/json and text/event-stream.'); return; }
    if (!String(request.headers['content-type'] ?? '').toLowerCase().startsWith('application/json')) { plain(request, response, 415, 'Expected application/json'); return; }

    try {
      const body = await readBody(request);
      let message;
      try { message = JSON.parse(body); }
      catch { json(request, response, 400, { jsonrpc: '2.0', id: null, error: { code: -32700, message: 'Parse error' } }); return; }
      ...
      if (method === 'initialize') {
        if (incomingSessionId) { plain(request, response, 400, 'Initialization must not include MCP-Session-Id.'); return; }
        if (sessions.size >= maxSessions) { plain(request, response, 429, 'Too many active MCP sessions.'); return; }
        const session = createSwitchboardMcpSession();
        const result = await session.handle(message);
        if (result?.error) { json(request, response, 400, result); return; }
      ...
      if (!incomingSessionId) { plain(request, response, 400, 'MCP-Session-Id is required after initialization.'); return; }
      const state = sessions.get(incomingSessionId);
      if (!state) { response.writeHead(404, corsHeaders(request)).end(); return; }
      const protocol = protocolHeader(request);
      if (!protocol) { plain(request, response, 400, 'MCP-Protocol-Version is required after initialization.'); return; }
      if (!SUPPORTED_PROTOCOL_VERSIONS.has(protocol) || protocol !== state.protocolVersion) { plain(request, response, 400, 'Unsupported or mismatched MCP protocol version.'); return; }
      state.lastSeen = Date.now();
      const result = await state.session.handle(message);
      const responseHeaders = { 'mcp-session-id': incomingSessionId };
      if (result === null) { response.writeHead(202, { 'cache-control': 'no-store', ...corsHeaders(request), ...responseHeaders }).end(); return; }
