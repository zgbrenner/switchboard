...
import { enhanceDecision, applyProfileFloor } from '../enhance.mjs';
...
import {
  COMPARISON_INPUT_SCHEMA, EVALUATION_INPUT_SCHEMA, INVENTORY_INPUT_SCHEMA, ROUTE_INPUT_SCHEMA,
  normalizeComparisonArguments, normalizeEvaluationArguments, normalizeInventoryArguments, normalizeRouteArguments,
} from '../schema.mjs';
import {
  COMPARISON_OUTPUT_SCHEMA, EVALUATION_OUTPUT_SCHEMA, EXPLAIN_OUTPUT_SCHEMA,
  INVENTORY_OUTPUT_SCHEMA, PREFERENCE_STATE_OUTPUT_SCHEMA,
...
export function toolDefinitions() {
  return [
    {
      name: 'route_request', title: 'Route AI Request',
      ...
      inputSchema: ROUTE_INPUT_SCHEMA, outputSchema: ROUTE_OUTPUT_SCHEMA_V05, annotations: READ_ONLY,
    },
    { name: 'explain_route', title: 'Explain Route', description: 'Route a request and return a concise explanation plus structured evidence.', inputSchema: ROUTE_INPUT_SCHEMA, outputSchema: EXPLAIN_OUTPUT_SCHEMA, annotations: READ_ONLY },
    { name: 'compare_routes', title: 'Compare Routes', description: 'Compare two to eight policy or profile variants for the same request.', inputSchema: COMPARISON_INPUT_SCHEMA, outputSchema: COMPARISON_OUTPUT_SCHEMA, annotations: READ_ONLY },
    { name: 'simulate_policy', title: 'Simulate Policies', description: 'Simulate selected policy or profile variants without changing state.', inputSchema: COMPARISON_INPUT_SCHEMA, outputSchema: COMPARISON_OUTPUT_SCHEMA, annotations: READ_ONLY },
    { name: 'validate_model_inventory', title: 'Validate Model Inventory', description: 'Validate and summarize a provider-independent model inventory without routing a prompt.', inputSchema: INVENTORY_INPUT_SCHEMA, outputSchema: INVENTORY_OUTPUT_SCHEMA, annotations: READ_ONLY },
    { name: 'evaluate_router', title: 'Evaluate Router', description: 'Evaluate up to 100 labeled routing cases and return safety and quality metrics. Baseline metrics exclude learned preferences by default.', inputSchema: EVALUATION_INPUT_SCHEMA, outputSchema: EVALUATION_OUTPUT_SCHEMA, annotations: READ_ONLY },
    { name: 'record_override', title: 'Record Aggregate Override', description: 'Record category-level upgrade or downgrade feedback. Does not accept or store prompt text.', inputSchema: OVERRIDE_INPUT_SCHEMA, outputSchema: PREFERENCE_STATE_OUTPUT_SCHEMA, annotations: WRITE_AGGREGATE },
    { name: 'get_preference_state', title: 'Get Aggregate Preference State', description: 'Read category-level aggregate preference weights and counters.', inputSchema: EMPTY_INPUT_SCHEMA, outputSchema: PREFERENCE_STATE_OUTPUT_SCHEMA, annotations: READ_ONLY },
    { name: 'reset_preference_state', title: 'Reset Aggregate Preference State', description: 'Delete all aggregate preference weights and counters.', inputSchema: EMPTY_INPUT_SCHEMA, outputSchema: PREFERENCE_STATE_OUTPUT_SCHEMA, annotations: RESET_AGGREGATE },
  ];
}
