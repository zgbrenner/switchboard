...
function parseFiles(value) {
  ...
  return value.map((raw, index) => {
    ...
    if (file.detectedType !== undefined && !FILE_TYPES.has(file.detectedType)) throw new Error(`files[${index}].detectedType is invalid.`);
    const detectedType = file.detectedType ?? 'unknown';
    ...
    const vision = optionalBoolean(caps.vision, `files[${index}].capabilities.vision`, detectedType === 'image');
    const longContext = optionalBoolean(caps.longContext, `files[${index}].capabilities.longContext`, textLength > 24_000 || size > 8_000_000);
...
function parseModels(value) {
  ...
  return value.map((raw, index) => {
    ...
    const id = string(model.id, `availableModels[${index}].id`, { min: 1, max: 200, nonWhitespace: true });
    ...
    if (!Array.isArray(effortLevels) || effortLevels.length > 4 || effortLevels.some((effort) => !EFFORTS.has(effort)) || new Set(effortLevels).size !== effortLevels.length) {
      throw new Error(`availableModels[${index}].effortLevels is invalid or contains duplicates.`);
    }
    return {
      id,
      ...(model.title === undefined ? {} : { title: string(model.title, `availableModels[${index}].title`, { min: 1, max: 200, nonWhitespace: true }) }),
      ...(model.family === undefined ? {} : { family: string(model.family, `availableModels[${index}].family`, { min: 1, max: 100, nonWhitespace: true }) }),
      tier,
      effortLevels,
      ...
      available: optionalBoolean(model.available, `availableModels[${index}].available`, true),
    };
  });
}
...
const MODEL_INPUT_SCHEMA = {
  ...
  properties: {
    ...
    family: { type: 'string', minLength: 1, maxLength: 100, pattern: '.*\\S.*' },
...
export const ROUTE_INPUT_SCHEMA = {
  ...
  properties: {
    prompt: { type: 'string', minLength: 1, maxLength: 64_000, pattern: '.*\\S.*', description: 'The current user request to classify.' },
    context: {
      type: 'array', maxItems: 8, description: 'Up to eight recent turns, used only for this routing decision.',
      items: { type: 'object', additionalProperties: false, required: ['role', 'text'], properties: { role: { type: 'string', enum: ['user', 'assistant'] }, text: { type: 'string', minLength: 1, maxLength: 12_000, pattern: '.*\\S.*' } } },
    },
    files: {
      ...
      items: {
        ...
        properties: {
          name: { type: 'string', minLength: 1, maxLength: 512, pattern: '.*\\S.*' }, size: { type: 'integer', minimum: 0, maximum: 1_000_000_000 },
          detectedType: { type: 'string', enum: [...FILE_TYPES] }, mediaType: { type: 'string', minLength: 1, maxLength: 200, pattern: '.*\\S.*' },
    ...
    currentModelId: { type: 'string', minLength: 1, maxLength: 200, pattern: '.*\\S.*', description: 'Current model identifier, used to avoid unnecessary switches when it remains adequate.' },
  },
};
