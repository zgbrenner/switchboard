...
test('route_request resolves an optional host model inventory without selecting under-capable models', async () => {
  ...
  assert.equal(
    resolution.alternatives.some((model) => model.id === 'deep-no-code'),
    false,
  );
});
...
test('model resolution respects policy and existing-model continuity when choices are adequate', async () => {
  ...
  const fast = await session.handle(
    request(2, 'tools/call', {
      name: 'route_request',
      arguments: { prompt: 'Explain this familiar concept clearly.', policy: 'fast', availableModels },
    }),
  );
  assert.equal(fast.result.structuredContent.modelResolution.recommended.id, 'balanced');

  const best = await session.handle(
    request(3, 'tools/call', {
      name: 'route_request',
      arguments: { prompt: 'Explain this familiar concept clearly.', policy: 'best', availableModels },
    }),
  );
  assert.equal(best.result.structuredContent.modelResolution.recommended.id, 'max');

  const continuity = await session.handle(
    request(4, 'tools/call', {
      name: 'route_request',
      arguments: {
        prompt: 'Explain this familiar concept clearly.',
        policy: 'balanced',
        currentModelId: 'balanced',
        context: [{ role: 'user', text: 'We are already discussing this topic.' }],
        availableModels,
      },
    }),
  );
...
test('empty or incompatible model inventories return explicit resolution states', async () => {
  ...
  const incompatible = await session.handle(
    request(3, 'tools/call', {
      name: 'route_request',
      arguments: {
...
test('server metadata resource, additive output schema, and policy completion are discoverable', async () => {
  ...
  assert.equal(
    resources.result.resources.some((resource) => resource.uri === 'switchboard://server'),
    true,
  );
  assert.equal(
    resources.result.resources.some((resource) => resource.uri === 'switchboard://api'),
    true,
  );
  ...
  const completion = await session.handle(
    request(5, 'completion/complete', {
      ref: { type: 'ref/prompt', name: 'route_before_answering' },
      argument: { name: 'policy', value: 'b' },
    }),
  );
...
test('HTTP transport creates secure sessions and requires session and protocol headers', async (t) => {
  ...
  const initialize = await fetch(url, {
    ...
    headers: httpHeaders(),
    body: JSON.stringify(
      request(1, 'initialize', { protocolVersion: '2025-11-25', capabilities: {}, clientInfo: { name: 'http', version: '1' } }),
    ),
  });
  ...
  const missingSession = await fetch(url, {
    method: 'POST',
    headers: httpHeaders({ 'mcp-protocol-version': '2025-11-25' }),
  ...
  const missingVersion = await fetch(url, {
    method: 'POST',
    headers: httpHeaders({ 'mcp-session-id': sessionId }),
...
test('HTTP transport validates Accept, authentication, expiry, and non-loopback safety', async (t) => {
  ...
  assert.equal(
    (
      await fetch(url, {
        method: 'POST',
        headers: { ...httpHeaders({ authorization: 'Bearer test-secret' }), accept: 'application/json' },
        body,
      })
    ).status,
    406,
